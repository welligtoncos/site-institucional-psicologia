"""
Publisher RabbitMQ para eventos de negócio.
"""

from __future__ import annotations

import json
import logging
from datetime import datetime, timezone
from typing import Any
from uuid import uuid4

import pika
from pika.exceptions import AMQPError

from app.core.config import get_settings
from app.messaging.constants import (
    BUSINESS_DLX_EXCHANGE,
    BUSINESS_DLX_ROUTING_KEY,
    BUSINESS_DLQ_QUEUE,
    BUSINESS_EXCHANGE,
    BUSINESS_EXCHANGE_TYPE,
)

logger = logging.getLogger(__name__)


class BusinessEventPublisher:
    def __init__(self) -> None:
        self._settings = get_settings()

    def publish(
        self,
        *,
        event_type: str,
        actor: str,
        resource_type: str,
        resource_id: str | None = None,
        data: dict[str, Any] | None = None,
        metadata: dict[str, Any] | None = None,
        correlation_id: str | None = None,
    ) -> None:
        payload = {
            "event_id": str(uuid4()),
            "event_type": event_type,
            "occurred_at": datetime.now(timezone.utc).isoformat(),
            "source": "app_backend",
            "actor": actor,
            "correlation_id": correlation_id or str(uuid4()),
            "resource_type": resource_type,
            "resource_id": resource_id,
            "data": data or {},
            "metadata": metadata or {},
        }

        params = pika.URLParameters(self._settings.rabbitmq_url)
        try:
            with pika.BlockingConnection(params) as connection:
                channel = connection.channel()
                self._declare_topology(channel)
                channel.basic_publish(
                    exchange=BUSINESS_EXCHANGE,
                    routing_key=event_type,
                    body=json.dumps(payload, ensure_ascii=False, default=str),
                    properties=pika.BasicProperties(
                        content_type="application/json",
                        delivery_mode=2,
                    ),
                )
                logger.info("Evento de negócio publicado: %s", event_type)
        except AMQPError:
            logger.exception("Falha ao publicar evento de negócio: %s", event_type)
            raise

    @staticmethod
    def _declare_topology(channel: pika.adapters.blocking_connection.BlockingChannel) -> None:
        channel.exchange_declare(
            exchange=BUSINESS_EXCHANGE,
            exchange_type=BUSINESS_EXCHANGE_TYPE,
            durable=True,
        )
        channel.exchange_declare(
            exchange=BUSINESS_DLX_EXCHANGE,
            exchange_type="direct",
            durable=True,
        )
        channel.queue_declare(queue=BUSINESS_DLQ_QUEUE, durable=True)
        channel.queue_bind(
            queue=BUSINESS_DLQ_QUEUE,
            exchange=BUSINESS_DLX_EXCHANGE,
            routing_key=BUSINESS_DLX_ROUTING_KEY,
        )
